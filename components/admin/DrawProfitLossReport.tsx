...
const DrawProfitLossReport: React.FC<{ draw: Draw; conditionFilter: 'ALL' | 'FIRST' | 'SECOND' }> = ({ draw, conditionFilter }) => {
    ...
    const processedData = useMemo(() => {
        ...
        for (const bet of relevantBets) {
            ...
            // For this report, we only care about the potential prize for the bet's specific condition
            const potentialPrize = getPrizeForBet(bet, client);

            const expandAndApply = (prefix: string, length: number, sourceSetKey: keyof ReportRow) => {
            ...
            switch (bet.gameType) {
                case GameType.FourDigits: expandAndApply(bet.number, 4, 'source4D'); break;
                case GameType.ThreeDigits: expandAndApply(bet.number, 3, 'source3D'); break;
                case GameType.TwoDigits: expandAndApply(bet.number, 2, 'source2D'); break;
                case GameType.OneDigit: expandAndApply(bet.number, 1, 'source1D'); break;
                // Positional bets are complex and require a different logic
                case GameType.Positional:
                    const digits = bet.number.split('').map((char, index) => (char !== 'X' ? { char, index } : null)).filter(Boolean);
                    const wildcards = 4 - (digits.length as number);
                    const combinations = 10 ** wildcards;

                    for(let i=0; i<combinations; i++) {
                        let tempNumber = bet.number.split('');
                       ...
                       {paginatedData.map(item => {
                           ...
                           return (
                            <tr key={item.playedNumber} className={`hover:bg-brand-secondary/30 ${isWinner ? 'bg-yellow-400/20' : ''}`}>
                                <td className={`px-4 py-2 font-mono font-bold ${isWinner ? 'text-brand-primary' : 'text-brand-text'}`}>{item.playedNumber}</td>
